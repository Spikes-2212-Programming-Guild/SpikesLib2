...
public class PIDOnTalon extends CommandBase {
    ...
    public PIDOnTalon(Subsystem subsystem, PIDTalon talon, Supplier<Double> kF, Supplier<Double> maxSpeed,
    ...
    public PIDOnTalon(GenericSubsystem genericSubsystem, PIDTalon talon, Supplier<Double> kF, ControlMode mode,
    ...
    public PIDOnTalon(GenericSubsystem genericSubsystem, PIDTalon talon, Supplier<Double> kF,
                      Supplier<Double> setpoint, int timeout) {
        addRequirements(genericSubsystem);
        this.talon = talon;
        this.kF = kF;
    ...
    public PIDOnTalon(Subsystem subsystem, PIDTalon talon, Supplier<Double> maxSpeed,
                      Supplier<Double> minSpeed, ControlMode mode, Supplier<Double> setpoint,
                      int timeout, Predicate<Double> canMove) {
        addRequirements(subsystem);
        this.talon = talon;
    ...
    public PIDOnTalon(GenericSubsystem genericSubsystem, PIDTalon talon, ControlMode mode,
                      Supplier<Double> setpoint, int timeout) {
        addRequirements(genericSubsystem);
        this.talon = talon;
        this.kF = ()->0.0;
    ...
    public PIDOnTalon(GenericSubsystem genericSubsystem, PIDTalon talon, Supplier<Double> setpoint, int timeout) {
        addRequirements(genericSubsystem);
        this.talon = talon;
        this.kF = ()->0.0;
        this.maxSpeed = genericSubsystem.maxSpeed;
    ...
    public void initialize() {
        talon.configureLoop(kF, maxSpeed, minSpeed, timeout);
    }

    @Override
    public void execute() {
        talon.pidSet(mode, setpoint.get(), timeout);
    }
    ...
    public boolean isFinished() {
        ...
        return Timer.getFPGATimestamp() - lastTimeNotOnTarget >= talon.getSettings().getWaitTime();
    }
}
